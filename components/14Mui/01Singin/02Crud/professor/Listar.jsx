...
const Listar = () => {
    ...
    useEffect(
        ()=>{
            ...
            axios.get("http://localhost:3001/professor")
            .then(
                (response)=>{
    ...
    function deleteProfessorById(id) {
        if(window.confirm("Deseja Excluir ? " + id)){
            ...
            axios.delete(`http://localhost:3001/professor/${id}`)
            .then((response)=>{
                                ...
                                (professor) => {
                                    return (
                                        <StyledTableRow key={professor.id}>
                                            ...
                                            <StyledTableCell>{professor.data.nome}</StyledTableCell>
                                            <StyledTableCell>{professor.data.curso}</StyledTableCell>
                                            <StyledTableCell>{professor.data.titulacao}</StyledTableCell>
                                            <StyledTableCell>
                                                <Box>
